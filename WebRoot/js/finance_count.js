function update_record(id) {
	if (window.navigator.userAgent.indexOf("MSIE") >= 1) {
		$.jBox("iframe:finance_jsp/update_record.jsp?id=" + id, {
			title : "更新信息",
			width : 600,
			height : 550,
			top : '5px',
...
function refresh() {
	...
	var recordItemId = $("#recordItemId").val();
	var yearth = $("#yearth").val();
	var date = $("#date").val();
	var bill = $("#bill").val();
	var goUp = $("#goUp").val();
	var goUpBill = $("#goUpBill").val();
	var goUpWhere = $("#goUpWhere").val();
	var goDown = $("#goDown").val();
	var goDownBill = $("#goDownBill").val();
	var goDownWhere = $("#goDownWhere").val();
	var goMoneyWhere = $("#goMoneyWhere").val();
	var remark = $("#remark").val();
	DatagridFactory.createDatagrid({
		type : "Pagination",
		id : "#financecount_data",
		action : "/zk/getDatasR",
		rownumbers : true,
		data : {
			"idCard" : idCard,
			...
			"goUpWhere" : goUpWhere,
			"goDown" : goDown,
			"goDownBill" : goDownBill,
			"goDownWhere" : goDownWhere,
			"goMoneyWhere" : goMoneyWhere,
			"remark" : remark,
			"d" : new Date().getTime()
		},
		columns : [ [ {
			...
			field : "stuBExamNumber",
			title : "本科考籍号",
			hidden : true,
			width : 80
		}, {
			field : "stuZSchoolName",
			title : "专科学校",
			width : 100
		}, {
			field : "stuZMajorName",
			...
			field : "stuZExamNumber",
			title : "专科考籍号",
			hidden : true,
			width : 80
		}, {
			field : "recordItemName",
			title : "交费项目",
			align : 'center',
			width : 80
		}, {
			...
			formatter : function(val, rowData, rowIndex) {
				return "<a href=javascript:delete_record('"+val+"');>删除</a>";
			}
		}
		] ]
	});
